...
*/
	...
	public void loadData() {
		try {
			...
			// så if data[3] är int, så har personen inga djur.
			// om data[4] är int, så har personen ett djur.
			// om data[5] är int, så har personen två djur.
			for (String line : lines) {
				data = line.split(",");
				if(!line.isEmpty()){
					if (data[3].substring(0, 1).matches("^[0-9]$") || data[3] == "none") {
						// Inga djur
						PersonBean p = new PersonBean(data[0], data[1], data[2], "none", new Integer(data[3]));
						personData.add(p);
					}
					else if (data[4].substring(0, 1).matches("^[0-9]$")) {
						// Personen har ett djur
						PersonBean p = new PersonBean(data[0], data[1], data[2], data[3], new Integer(data[4]));
						personData.add(p);
					}
					else if (data[5].substring(0, 1).matches("^[0-9]$")) {
						// Personen har två djur
